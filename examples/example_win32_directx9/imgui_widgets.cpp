...
bool ImGui::ValorantSubButton(const char* label, bool isvalid, const ImVec2& xsize)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::ValorantTopButton(const char* label, bool isvalid, const ImVec2& xsize)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if(isvalid)
    { 
    RenderFrame(ImVec2(bb.Min.x + ((bb.GetWidth() / 2)*(1- it_hover->second.w))  ,bb.Min.y ), ImVec2(bb.Max.x - ((bb.GetWidth() / 2) * (1 - it_hover->second.w)),bb.Max.y), GetColorU32(ImVec4{ 1,1,1,(0.10f* it_hover->second.w) }));
    for (int i = 0; i < 5; i++)
    {
    ...
    PushStyleColor(ImGuiCol_Text, ImVec4{ 234.f / 255.f,238.f / 255.f,178.f / 255.f,1 });
        RenderArrow(ImVec2(bb.Min.x + (bb.GetWidth() / 2) - 6.6f, bb.Max.y - 2), ImGuiDir_Down, 0.6f);
        PopStyleColor();
...
bool ImGui::Checkbox(const char* label, bool* v, float col2[4])
{
    ...
    if (*v)
    {
        ...
        RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad + 2.8f, pad-0.8f), GetColorU32(ImVec4{ 44.f / 255.f,52.f / 255.f,56.f / 255.f,it_hover->second.w }), square_sz - pad * 2.0f);
    }
...
static ImVec2 CalcSizeContents(ImGuiWindow* window)
{
    ...
    if (window->Hidden && window->HiddenFramesForRenderOnly == 0 && window->HiddenFramesForRenderOnly > 0)
        return window->ContentSize;
...
{
    ImGuiContext& g = *GImGui;
    if (g.NextWindowData.SizeCond != 0)
    {
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    // Render shape
    const ImU32 frame_col = GetColorU32(hovered ? ImVec4{1,1,1,0.35} : ImVec4{1,1,1,0.15});
    const float value_x2 = ImMax(bb.Min.x, bb.Max.x - arrow_size);
    RenderNavHighlight(bb, id);
    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRectFilled(bb.Min, bb.Max, frame_col, style.FrameRounding, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersLeft);
    RenderArrow(ImVec2(value_x2,bb.Min.y + (bb.GetHeight() / 2)- 4.00f), ImGuiDir_Down, 0.80f);
    // Custom preview
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    static std::map<ImGuiID, ImVec4> hover_animation;
    auto it_hover = hover_animation.find(popup_id);
    if (it_hover == hover_animation.end())
    {
        hover_animation.insert({ popup_id, hover_off });
        it_hover = hover_animation.find(popup_id);
    }
    if (popup_open)
    {
        ImVec4 to = hover_act;
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(w, 0.0f), ImVec2(FLT_MAX, (CalcMaxPopupHeightFromItemCount(popup_max_height_in_items) * it_hover->second.w)));
    }
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ...
    for (int i = 0; i < items_count; i++)
    {
        ...
        if (i % 2 == 0)
        {
            PushStyleColor(ImGuiCol_Header, ImVec4{191.f /255.f,191.f / 255.f ,191.f / 255.f ,1.f});
            PushStyleColor(ImGuiCol_HeaderHovered, ImVec4{ 211.f / 255.f,215.f / 255.f ,161.f / 255.f ,1.f });
            PushStyleColor(ImGuiCol_HeaderActive, ImVec4{191.f /255.f,191.f / 255.f ,191.f / 255.f ,1.f});
        }
        else if (i % 2 == 1)
        {
            PushStyleColor(ImGuiCol_Header, ImVec4{ 204.f / 255.f,204.f / 255.f ,204.f / 255.f ,1.f });
            PushStyleColor(ImGuiCol_HeaderHovered, ImVec4{ 211.f / 255.f,215.f / 255.f ,161.f / 255.f ,1.f });
            PushStyleColor(ImGuiCol_HeaderActive, ImVec4{ 214.f / 255.f,214.f / 255.f ,214.f / 255.f ,1.f });
        }
        if (item_selected)
        {
            PushStyleColor(ImGuiCol_Header, ImVec4{ 222.f / 255.f,226.f / 255.f ,169.f / 255.f ,1.f });
            PushStyleColor(ImGuiCol_HeaderHovered, ImVec4{ 211.f / 255.f,215.f / 255.f ,161.f / 255.f ,1.f });
            PushStyleColor(ImGuiCol_HeaderActive, ImVec4{ 242.f / 255.f,246.f / 255.f ,189.f / 255.f ,1.f });
        }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 0.6f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImVec4{ 1,1,1,0.35f }), true, 12);
    ...
    if (value_changed)
        it_filled->second = clamp(it_filled->second - 2.2f * ImGui::GetIO().DeltaTime, 0.9f, 1.f);
    else if(hovered && it_filled->second >= 0.9f)
        it_filled->second = clamp(it_filled->second - 2.2f * ImGui::GetIO().DeltaTime, 0.9f, 1.f);
    else
        it_filled->second = clamp(it_filled->second + (2.2f * ImGui::GetIO().DeltaTime * ((hovered) ? 1.f : -1.f)), 0.0f, 1.f);
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 3, frame_bb.Min.y + 4), frame_bb.Max - ImVec2(3, 4), GetColorU32(ImVec4{1,1,1,1}), 12);
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 3, frame_bb.Min.y + 4), frame_bb.Max - ImVec2(3, 4), GetColorU32(ImVec4{ 238.f / 255.f, 241.f / 255.f, 193.f / 255.f,it_filled->second }), 12);

    // Render grab
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddCircleFilled(grab_bb.GetCenter(), 7, GetColorU32(ImVec4{0.92f,0.92f,0.92f,1}), 32);
        window->DrawList->AddCircleFilled(grab_bb.GetCenter(), 7, GetColorU32(ImVec4{ 238.f / 255.f, 235.f / 255.f, 176.f / 255.f,it_filled->second }), 32);
        window->DrawList->AddCircle(grab_bb.GetCenter(), 7 + (5* it_filled->second), GetColorU32(ImVec4{ 238.f / 255.f, 235.f / 255.f, 176.f / 255.f,it_filled->second }), 32);
    ...
    RenderTextClipped(ImVec2(frame_bb.Max.x+ 5,frame_bb.Min.y), frame_bb.Max+ ImVec2(120,0), value_buf, value_buf_end, NULL, ImVec2(0.f, 0.5f));
    PopStyleColor();
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x + 5,frame_bb.Min.y - (frame_bb.GetHeight() +2)), label);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    else
    {
      ...
      /*  if (col_source.w < 1.0f)
            ...
            window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), 12);
    }
    RenderNavHighlight(bb, id);
